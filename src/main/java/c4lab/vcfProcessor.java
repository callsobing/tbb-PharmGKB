...
public class vcfProcessor implements Serializable {
    public static void main(String[] args) throws IOException {
        ...
        SparkConf conf = new SparkConf().setAppName("yat-parsing");
        ...
        String headerWithSamples = file.filter(line -> line.startsWith("#CHROM")).reduce((s1, s2) -> s1 + "\n" + s2);
        List<String> sampleIds = Arrays.asList(headerWithSamples.split("\\t")).subList(9, headerWithSamples.split("\\t").length - 1);
        ...
        JavaPairRDD<String, String> aa = vctx.mapPartitionsToPair(new PairFlatMapFunction<Iterator<VariantContext>, String, String>() {
            @Override
            public Iterable<Tuple2<String, String>> call(Iterator<VariantContext> variantContextIterator) throws Exception {
...
//            }
//        });
        aa.saveAsTextFile("/root/pgkb/pgkb_vip.1kg.af");
    }
}
